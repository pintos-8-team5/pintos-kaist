...
int parsing(char *f_name, char **argv)
{
	char *token;
	char *save_ptr;
	int argc = 0;
...
void load_to_stack(int argc, char **argv, struct intr_frame *if_)
{
	char *argv_addr[50]; // 배열 길이 몇으로 해야할지 모르겠음
	...
	// if (argc % 8 != 0)
	// {
	//	number_of_padding = ROUND_UP(argc, 8) - argc;
	// }
	...
	*(uint64_t *)if_->rsp = 0;
	...
	// if_->rsp -= 8;
	// memcpy((void *)if_->rsp, argv, 8);

	// if_->rsp -= 4;
	// memcpy((void *)if_->rsp, argc, 4);

	// if_->rsp -= 8;

	// uint64_t null = 0;
	// memcpy((void *)if_->rsp, &null, sizeof(uint64_t));

	uint64_t argv_base = (uint64_t)if_->rsp;
	if_->rsp -= 8;
	*(uint64_t *)if_->rsp = argv_base;
